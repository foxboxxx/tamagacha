...
void draw_char(char c, uint8_t x, uint8_t y, uint16_t text_color, uint16_t bg_color) {
    int font_idx = ((int)c) * 8; 
    
    uint16_t buf[64]; 
    int buf_idx = 0;

    for (int row = 0; row < 8; row++) {
        unsigned char row_data = console_font_8x8[font_idx + row];
        for (int col = 0; col < 8; col++) {
            if ((row_data >> (7 - col)) & 0b1) {
                buf[buf_idx++] = text_color; 
            } else {
                buf[buf_idx++] = bg_color;   
            }
        }
    }
    st7735_fill_window(buf, x, x + 8 - 1, y, y + 8 - 1);
}
...
void draw_string(const char* str, uint8_t x, uint8_t y, uint16_t text_color, uint16_t bg_color) {
    ...
    int i = 0;
    while (str[i] != '\0') {
        draw_char(str[i], current_x, y, text_color, bg_color);
        current_x += 8;
        i++;
    }
}

// font from https://github.com/idispatch/raster-fonts/blob/master/font-8x8.c, ty!
