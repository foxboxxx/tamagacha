// JULIAN NOTE: this file might have to be in libpi/src. The README for lab 8 has conflicting information.
...
static inline void *vector_base_get(void) {
    ...
    // section 3-121 of arm v1176 manual
	uint32_t out;
...
static inline void vector_base_set_raw(uint32_t v) {
    ...
    // prefetch flush, docs (table 3-75 in arm 1176 manual) say data is SBZ
    uint32_t in = 0;
    asm volatile("MCR p15, 0, %0, c7, c5, 4" :: "r"(in));
}
...
static inline int vector_base_chk(void *vector_base) {
    ...
    // jreed: needs to be 32 bit aligned
    if (((unsigned) vector_base & 31) != 0) {
        return 0;
    }
    return 1;
}
