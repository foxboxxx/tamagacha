...
void uart_init(void) {
    ...
    // disable TX/RX
    // PUT32(AUX_MU_CNTL_REG, GET32(AUX_MU_CNTL_REG) & ~3);
    PUT32(AUX_MU_CNTL_REG, 0);

    // find and clear state as needed
    // disable interrupts
    ...
    // baudrate = system clock  / (8 * (baudrate_reg + 1)), target - 115200
    //
    // PUT32(AUX_MU_BAUD, 270);
    PUT32(AUX_MU_BAUD, 67); // increase baud rate 
    ...
    // NOTE FROM STAFF: for cross-checking: make sure write UART 
    // addresses in order
...
int uart_has_data(void) {
    int ret = (GET32(AUX_MU_STAT_REG) & 1) != 0;
    return ret;
}
...
int uart_get8(void) {
    dev_barrier();
    while (!uart_has_data()){}
    int ret = GET32(AUX_MU_IO_REG) & 0b11111111;
    dev_barrier();
    return ret;
}
...
int uart_can_put8(void) {
    int ret = (GET32(AUX_MU_STAT_REG) & 0b10) != 0;
    return ret;
}
...
int uart_tx_is_empty(void) {
    // return (GET32(AUX_MU_LSR_REG) & 96) == 0;
    dev_barrier();
    int ret = GET32(AUX_MU_STAT_REG) & 512;
    dev_barrier();
    return ret;
}
